...
class DanteNotificationService(DanteMulticastService):
    ...
    def _receive(
        ...
        #  0- 1: `\xFF\xFF`
        #  2- 3: Total Length of Message ## length = bytes2int(message[2:4])
        #  4- 5: Sequence ID             ## seq_id = bytes2int(message[4:6])
        #  6- 7: null hextet
        #  8-13: MAC Address             ## mac_addr = decode_mac_address(message[8:14])
        # 14-15: null hextet
        # 16-23: `Audinate`
        # 24-25: `\x07\x31` (as integers: either 7,49 or 1841) or `\x07\x2e` (7,46 or 1838)
        notification_id = bytes2int(message[26:28])
    ...
    def get_channels_affected(self, payload: bytes) -> list[int]:
        channels_affected = []
        for bank_idx in range(0, bytes2int(payload[0:2])):
            channels = bytes2int(payload[2 + bank_idx])
    ...
    def handle_128(self, device: DanteDevice, payload: bytes) -> None:
        ...
        * Payload contains new Sample Rate and list of possible Sample Rates
        ...
        sample_rate_count = bytes2int(payload[2:4])
    ...
    def handle_130(self, device: DanteDevice, payload: bytes) -> None:
        ...
        * Payload contains new PCM Encoding and list of possible encodings
        """
        # 0-3: \x00\x18
        pcm_encoding_count = bytes2int(payload[2:4])
    ...
    def handle_4103(self, device: DanteDevice, payload: bytes) -> None:
        ...
        # 2-3: null hextet
        enabled = bool(bytes2int(payload[0:2]))
        LOGGER.debug(
            "%s has changed its AES67 status, which is now %s",
            device.name,
