...
class _DanteChannel:
    ...
    async def _set_name(
        ...
        payload = (
            preamble,
            struct.pack('>H', self._app.arc_service.SERVICE_HEADER_LENGTH + len(preamble) + 2),
            # packet traces had null hextets here, for padding(?): 23/30/2 for RX; 47/45/0 for TX (2.8.9, 2.8.1, 2.7.x)
            encode_string(new_name),
        )
        response = await self._app.arc_service.request(self._device, opcode, payload)
...
class DanteRxChannel(_DanteChannel):
    ...
    def name(self, new_name: str) -> None:
        ...
        self._app.arc_service.run_task(self._set_name(opcode, preamble, new_name))
    ...
    def subscribe(self, tx_channel: DanteTxChannel) -> None:
        ...
        body = (
            preamble,
            struct.pack('>H', string_idx),
            struct.pack('>H', string_idx + len(tx_channel_name_encoded)),
            padding,
...
class DanteTxChannel(_DanteChannel):
    ...
    def name(self, new_name: str) -> None:
        ...
        self._app.run_task(self._set_name(opcode, preamble, new_name))
